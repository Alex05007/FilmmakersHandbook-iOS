...
struct ContentView: View {
    ...
    var body: some View {
        if (isConnected.connectionDescription == "true") {
            view(request: URLRequest(url: URL(string: "https://gnets.myds.me/account/?deviceId=" + UIDevice.current.identifierForVendor!.uuidString)!)).previewLayout(.fixed(width: .infinity, height: .infinity))
                    .ignoresSafeArea()
        } else {
            VStack {
